...
export default function uiCustomization(pi: ExtensionAPI) {
  ...
  let semanticUi: ExtensionContext["ui"] | undefined;
  let themeRemovalTimers: Array<ReturnType<typeof setTimeout>> = [];

  const publishSemanticStatus = () => {
    if (!semanticUi) return;
    const parts: string[] = [];
    if (modelInfo.modelId) {
      ...
      parts.push(
        `${modelInfo.provider ? `${modelInfo.provider}/` : ""}${modelInfo.modelId} · ${modelInfo.thinking} · ${context} context · $${modelInfo.cost.toFixed(2)}`,
      );
    }
    if (gitInfo.branch) {
      parts.push(
        `${gitInfo.branch} · ${gitInfo.changedFiles} changed ${gitInfo.changedFiles === 1 ? "file" : "files"}`,
      );
    }
    semanticUi.setStatus(
      "ui-customization",
