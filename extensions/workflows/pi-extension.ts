...
export default function workflows(pi: ExtensionAPI) {
  ...
  let declarativeContext: ExtensionContext | undefined;
  let runsView: ReturnType<typeof createDeclarativeView> | undefined;
  let runView: ReturnType<typeof createDeclarativeView> | undefined;
  let agentView: ReturnType<typeof createDeclarativeView> | undefined;
  let selectedRunId: string | undefined;
  let selectedAgentIndex: number | undefined;
  let declarativeRefresh: ReturnType<typeof setInterval> | undefined;

  const declarativeEntries = () => {
    if (!declarativeContext) return [];
    return loadRunEntries(
      activeDetails(),
      declarativeContext.sessionManager.getSessionId(),
      sessionWorkflowRunIds(declarativeContext),
    );
  };

  const selectedEntry = () =>
    declarativeEntries().find((entry) => entry.runId === selectedRunId);

  const closeAgentView = () => {
    agentView?.close();
    agentView = undefined;
  ...
  const publishRuns = () => {
    if (!runsView) return;
    const entries = declarativeEntries();
    runsView.update({
      title: "Workflows",
      status: entries.some((entry) => entry.details.status === "running")
        ? "running"
        : "idle",
      blocks: [
        ...(entries.length === 0
          ? [
              {
                type: "text" as const,
                text: "No workflow runs are associated with this chat yet.",
                tone: "muted" as const,
              },
            ]
          : []),
        {
          ...
          columns: [
            ...
            { id: "progress", label: "Agents", align: "end" },
            { id: "elapsed", label: "Elapsed", align: "end" },
          ],
          rows: entries.map((entry) => {
            const { done, failed } = countStates(entry.details);
            return {
              id: entry.runId,
              cells: {
                run: entry.runId,
                name: entry.details.name ?? "Unnamed workflow",
                status: entry.details.status,
                progress: `${done + failed}/${entry.details.agents.length}`,
                elapsed: formatElapsed(
                  entry.details.startedAt,
                  entry.details.finishedAt,
                ),
              },
              tone: runStatusTone(entry.details.status),
            };
          }),
          ...(selectedRunId ? { selectedId: selectedRunId } : {}),
        } satisfies DeclarativeBlock,
      ],
      actions: [
        {
          id: "inspect",
          label: "Inspect",
          tone: "primary",
          selection: { blockId: "workflow-runs", required: true },
        ...
        },
        {
          ...
          confirm: {
            ...
            message:
              "Running workflow agents will be aborted and allowed a bounded cleanup period.",
          },
          disabled: entries.length === 0,
        },
  ...
  const publishRun = () => {
    if (!runView || !selectedRunId) return;
    ...
    if (!entry) {
      runView.update({
        ...
        blocks: [
          {
            type: "text",
            text: "This workflow run is unavailable.",
            tone: "error",
          },
        ],
    ...
    const phaseItems = phaseGroups(details, true).map((group) => ({
      id:
        group.title.replaceAll(/[^a-zA-Z0-9._:-]/g, "-").slice(0, 150) ||
        "phase",
      label: group.title,
      detail: `${group.agents.filter((agent) => agent.state !== "running").length}/${group.agents.length} settled`,
      tone:
        group.title === details.currentPhase
          ? ("info" as const)
          : ("default" as const),
    }));
    const blocks: DeclarativeBlock[] = [
      {
        ...
        entries: [
          { label: "Run", value: details.runId },
          {
            ...
            tone: runStatusTone(details.status),
          },
          { label: "Current phase", value: details.currentPhase ?? "—" },
          {
            label: "Agents",
            value: `${done} done · ${running} running · ${failed} failed`,
          },
          {
            label: "Elapsed",
          ...
          {
            label: "Usage",
            value: formatUsage(aggregateUsage(details.agents)) || "Unavailable",
          },
        ],
      },
      {
        type: "progress",
        value: done + failed,
        max: Math.max(details.agents.length, 1),
      ...
      ...(phaseItems.length
        ? [{ type: "list" as const, id: "workflow-phases", items: phaseItems }]
        : []),
      {
        type: "table",
        id: "workflow-agents",
        ...
        rows: details.agents.map((agent) => ({
          ...
          cells: {
            ...
            context: agentContext(agent) || "Unknown",
        ...
        ...(selectedAgentIndex !== undefined
          ? { selectedId: String(selectedAgentIndex) }
          : {}),
      },
      ...(details.error
        ? [
      ...
      ...(details.result !== undefined
        ? [
            {
              type: "code" as const,
              code: resultJson(details.result),
              language: "json",
              label: "Result",
            },
          ]
      ...
      actions: [
        {
          ...
          label: "Inspect agent",
          tone: "primary",
          selection: { blockId: "workflow-agents", required: true },
          disabled: details.agents.length === 0,
        },
        {
          id: "cancel",
          label: "Cancel run",
          tone: "danger",
          disabled: details.status !== "running",
          confirm: {
            title: "Cancel workflow?",
            message: "Every running agent in this workflow will be aborted.",
          },
        },
        { id: "save-report", label: "Save report" },
        { id: "back", label: "Back" },
      ],
    });
  };
  ...
  const publishAgent = () => {
    if (!agentView || selectedAgentIndex === undefined) return;
    const entry = selectedEntry();
    const agent = entry?.details.agents.find(
      (candidate) => candidate.index === selectedAgentIndex,
    );
    if (!entry || !agent) {
      agentView.update({
        ...
        blocks: [
          {
            type: "text",
            text: "This workflow agent is unavailable.",
            tone: "error",
          },
        ],
        actions: [{ id: "back", label: "Back" }],
    ...
    const lines = agent.transcript.flatMap((transcript) =>
      transcript.text.split("\n").map((line) => ({
        text: `${transcript.role}${transcript.name ? ` (${transcript.name})` : ""}: ${line.slice(0, 2_000)}`,
        timestamp: transcript.timestamp,
        tone: transcript.isError
          ? ("error" as const)
          : transcript.role === "thinking"
            ? ("muted" as const)
            : undefined,
      })),
    );
    agentView.update({
      title: `${agent.label} · ${entry.details.name ?? entry.runId}`,
      status:
        agent.state === "running"
          ? "running"
          : agent.state === "error"
      ...
      blocks: [
        {
          ...
          entries: [
            ...
            { label: "Context", value: agentContext(agent) || "Unknown" },
            {
              label: "Elapsed",
              value: formatElapsed(agent.startedAt, agent.finishedAt),
            },
          ],
        },
        {
  ...
  const cancelRun = async (entry: RunEntry | undefined) => {
    if (!entry || entry.details.status !== "running") return;
    const active = activeRuns.get(entry.runId);
    if (!active) return;
    active.controller.abort("Cancelled from Forker");
    await active.controller.settle({ abort: true });
    publishDeclarativeViews();
  };

  const openAgentView = (agentIndex: number) => {
    if (!declarativeContext || !selectedRunId) return;
    selectedAgentIndex = agentIndex;
    agentView?.close();
    agentView = createDeclarativeView(pi, declarativeContext, {
      extensionId: "forker.workflows",
      viewId: `agent:${selectedRunId}:${agentIndex}`,
    });
    agentView.onAction("back", closeAgentView);
    publishAgent();
  };

  const openRunView = (runId: string) => {
    if (!declarativeContext) return;
    closeRunView();
    selectedRunId = runId;
    runView = createDeclarativeView(pi, declarativeContext, {
      extensionId: "forker.workflows",
      viewId: `run:${runId}`,
    });
    runView.onAction("inspect-agent", ({ selection }) => {
      if (selection) openAgentView(Number(selection.itemId));
    });
    runView.onAction("cancel", async ({ cancelled }) => {
      if (!cancelled) await cancelRun(selectedEntry());
    });
    runView.onAction("save-report", () => {
      const entry = selectedEntry();
      if (!entry || !declarativeContext) return;
      ...
      declarativeContext.ui.notify(
        `Saved workflow report: ${shortenHome(target)}`,
        "info",
      );
    });
    runView.onAction("back", closeRunView);
    publishRun();
  };

  const openDeclarativeDashboard = (
    ctx: ExtensionContext,
    initialRunId?: string,
  ) => {
    closeDeclarativeViews();
    declarativeContext = ctx;
    runsView = createDeclarativeView(pi, ctx, {
      extensionId: "forker.workflows",
      viewId: "runs",
    });
    runsView.onAction("inspect", ({ selection }) => {
      if (selection) openRunView(selection.itemId);
    });
    runsView.onAction("cancel", async ({ selection, cancelled }) => {
      if (!cancelled && selection) {
        await cancelRun(
          declarativeEntries().find(
            (entry) => entry.runId === selection.itemId,
          ),
        );
      }
    });
    runsView.onAction("refresh", publishDeclarativeViews);
    runsView.onAction("close", closeDeclarativeViews);
    publishRuns();
    if (initialRunId) {
      const entry = declarativeEntries().find(
        (candidate) =>
          candidate.runId === initialRunId ||
          candidate.runId.endsWith(initialRunId),
      );
      if (entry) openRunView(entry.runId);
    }
  ...
  pi.registerCommand("workflows", {
    ...
    handler: async (rawArgs, ctx) => {
      ...
      if (supportsDeclarativeUi(ctx)) {
        lastUi = ctx.ui;
        openDeclarativeDashboard(ctx, arg || undefined);
        completedRuns = 0;
        failedRuns = 0;
        updateIndicator();
        return;
