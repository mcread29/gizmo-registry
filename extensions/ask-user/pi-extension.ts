...
export default function askUser(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      ...
      // Gizmo binds extensions with mode "json" but a dialog-capable UI
      // (ctx.hasUI), so the bridge path must not be limited to rpc mode.
      if (ctx.mode === "rpc" || ctx.hasUI) {
        const optionLabels = params.options.map(
          (option, index) => `${index + 1}. ${option.label}`,
        );
        const customLabel = `${params.options.length + 1}. Write my own answer…`;
        const selected = await ctx.ui.select(
          params.question,
          [...optionLabels, customLabel],
          { signal },
        );
        ...
        if (selected === customLabel) {
          const answer = await ctx.ui.input("Your answer", "", { signal });
          return answer === undefined
            ? reply(
                "User dismissed the question without answering. Do not assume an answer; proceed accordingly or ask differently.",
              )
            : reply(`User wrote their own answer: ${answer}`, answer, true);
        }
        const selectedIndex = optionLabels.indexOf(selected);
        if (selectedIndex < 0) return reply("Cancelled");
