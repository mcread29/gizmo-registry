...
test("the production timeout error names the tool and three-minute limit", () => {
  expect(
    new ToolCallTimeoutError("fixture_tool", SUBAGENT_TOOL_CALL_TIMEOUT_MS)
      .message,
  ).toBe('Tool call "fixture_tool" timed out after 3 minutes.');
});
...
test("a hung tool call fails clearly and receives an abort signal", async () => {
  ...
  await expect(pending).rejects.toThrow(ToolCallTimeoutError);
  await expect(pending).rejects.toThrow(
    'Tool call "hung_fixture" timed out after 10 ms.',
  );

  expect(executionSignal?.aborted).toBe(true);
  expect(executionSignal?.reason instanceof ToolCallTimeoutError).toBe(true);
});
...
test("the guard wraps each definition once and can discover later tools", () => {
  ...
  expect(firstWrappedExecute).not.toBe(firstExecute);
  ...
  expect(first.execute).toBe(firstWrappedExecute);
  expect(second.execute).not.toBe(secondExecute);
});
